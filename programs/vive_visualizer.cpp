...
int main(int argc, char ** argv) {
  ...
  TCLAP::ValueArg<std::string> config_arg("c", "config",
                                          ...
                                          true, "config.json", "string", cmd);
  TCLAP::ValueArg<int> port_arg("p", "port",
                                "The port on which vive messages are received (-1: no listener)",
                                false, -1, "port", cmd);
  TCLAP::ValueArg<std::string> log_arg("l", "log",
                                       "An optional path to existing vive logs",
                                       false, "", "path", cmd);
  TCLAP::ValueArg<int> time_arg("t", "time",
                                "time_offset between start of the two streams (replay)",
                                false, 0, "time offset[us]", cmd);
  TCLAP::ValueArg<float> x_arg("", "x_center",
                               "Value of field center along vive x_axis",
                               false, 0, "x_center[m]", cmd);
  TCLAP::ValueArg<float> y_arg("", "y_center",
                               "Value of field center along vive y_axis",
                               false, 0, "x_center[m]", cmd);
  TCLAP::ValueArg<float> z_arg("", "z_center",
                               "Value of field center along vive z_axis",
                               false, 0, "z_center[m]", cmd);
  TCLAP::ValueArg<float> roll_arg("", "roll",
                                  "Order: roll-pitch-yaw, applied after translation",
                                  false, 0, "roll[deg]", cmd);
  TCLAP::ValueArg<float> pitch_arg("", "pitch",
                                   "Order: roll-pitch-yaw, applied after translation",
                                   false, 0, "pitch[deg]", cmd);
  TCLAP::ValueArg<float> yaw_arg("", "yaw",
                                 "Order: roll-pitch-yaw, applied after translation",
                                 false, 0, "yaw[deg]", cmd);
